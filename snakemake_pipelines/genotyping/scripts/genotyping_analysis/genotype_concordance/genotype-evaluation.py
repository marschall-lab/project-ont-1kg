...
parser.add_argument('output', metavar='OUTPUT', help='output dcitionary')
...
parser.add_argument('--print_wrong_id', action='store_true', default=False, help="Print wrongly genotyped IDs.")
args = parser.parse_args()

output_dict = {
	'weighted_concordance': None,
	'genotype_concordance_all': None,
	'genotype_concordance_absent': None,
	'genotype_concordance_het': None,
	'genotype_concordance_hom': None,
	'typed': None,
	'total_baseline_variants': None,
	'total_unmatched': None,
	'precision': None,
	'recall': None
	}
...
second_column = [balanced_concordance, overall_concordance, conc_absent, conc_het, conc_hom, typed_all, all_present, unmatched, precision, recall]

for first,second in zip(first_column,second_column):
	output_dict[first] = second
	print('\t'.join([first,str(second)]))

if args.print_wrong_id:
	print('\n\n')
	for var_id,genotypes in id_to_genotypes.items():
		if (genotypes[0] != 3 and genotypes[1] != 3) and (genotypes[0] != genotypes[1]):
			print('0\t'+var_id)
		#if (genotypes[0] != 3 and genotypes[1] != 3) and (genotypes[0] == genotypes[1]):
		#	print('1\t'+var_id)
