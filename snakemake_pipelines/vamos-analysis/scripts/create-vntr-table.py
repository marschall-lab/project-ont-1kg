...
def run(stats=None, sites=None):
    ...
    print('#CHROM\tREF_START\tREF_END\tNUM_HAPS\tRU_LEN_AVG\tRU_LEN_STD\tNUM_UNIQUE_VNTRS\tMAX_RUS\tMIN_RUS\tMEDIAN_RUS\t1%_RUS\t5%_RUS\t25%_RUS\t75%_RUS\t95%_RUS\t99%_RUS\tUNIQUE_NUM_RUS\tMAX_BPS\tMIN_BPS\tMEDIAN_BPS\t1%_BPS\t5%_BPS\t25%_BPS\t75%_BPS\t95%_BPS\t99%_BPS')
    for site, value in data.items():
        #print(value, file=sys.stderr)
        n_samples = len(value[0])
        if n_samples == 0:
            print(f"{site[0]}\t{site[1]}\t{value[3]}\t{n_samples}\tNaN\tNaN\tNaN\tNaN\tNaN\tNaN\tNaN\tNaN\tNaN\tNaN\tNaN\tNaN\tNaN\tNaN\tNaN\tNaN\tNaN\tNaN\tNaN\tNaN")    
            continue
