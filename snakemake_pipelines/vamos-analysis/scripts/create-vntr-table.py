...
def run(stats=None, sites=None):
    ...
    print('#CHROM\tREF_START\tREF_END\tNUM_HAPS\tRU_LEN_AVG\tRU_LEN_STD\tNUM_UNIQUE_VNTRS\tMAX_RUS\tMIN_RUS\tMEDIAN_RUS\t25%_RUS\t75%_RUS\tUNIQUE_NUM_RUS\tMAX_BPS\tMIN_BPS\tMEDIAN_BPS\t25%_BPS\t75%_BPS')
    for site, value in data.items():
        ...
        print(f"{site[0]}\t{site[1]}\t{value[3]}\t{n_samples}\t{np.mean(ru_len)}\t{np.std(ru_len)}\t{unique_vntrs}\t{int(rus.max())}\t{int(rus.min())}\t{np.median(rus)}\t{np.percentile(rus, 25)}\t{np.percentile(rus, 75)}\t{unique_rus_num}\t{int(bps.max())}\t{int(bps.min())}\t{np.median(bps)}\t{np.percentile(bps, 25)}\t{np.percentile(bps, 75)}")
