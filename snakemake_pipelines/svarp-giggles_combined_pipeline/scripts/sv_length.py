...
if __name__ == '__main__':
    ...
    if args.vcf:
        ...
        writer = open(args.output+'/sv_length-temp.tsv', 'w')
        ...
        for n,v in enumerate(vcfs):
            ...
            for rec in reader.fetch():
                ...
                if gt[0] == 0  and gt[1] == 0:
                    ...
                    d[1][sv_type_to_index[sv_type]] += int(length)
                else:
                    # HET SV
                    d[0][sv_type_to_index[sv_type]] += int(length)
    ...
    # Plotting SV length sample-wise
    fig, ax = plt.subplots(figsize=(20,5), dpi=100)
    ax.bar(sample_list, hom_count_list, color=color_list)
    ax.set_ylabel('HOM SV length')
    ax.set_title('HOM SV lengths Sample-Wise')
    plt.tick_params(axis='x', which='both', bottom=False, top=False, labelbottom=False)
    plt.figlegend(handles, labels, framealpha=1, frameon=True)
    plt.tight_layout()
    plt.savefig(args.output+'/hom_length_sample-wise-temp.png')
    
    # Plotting Variant length sample-wise
    fig, ax = plt.subplots(figsize=(20,5), dpi=100)
    ax.bar(sample_list, het_count_list, color=color_list)
    ax.set_ylabel('HET SV length')
    ax.set_title('HET SV lengths Sample-Wise')
    plt.tick_params(axis='x', which='both', bottom=False, top=False, labelbottom=False)
    plt.figlegend(handles, labels, framealpha=1, frameon=True)
    plt.tight_layout()
    plt.savefig(args.output+'/het_length_sample-wise-temp.png')
    
    # Plotting SV length population-wise
    ...
    ax.set_title('HOM SV lengths Population-Wise', fontsize=15)
    plt.yticks(fontsize=15)
    plt.xticks(fontsize=15)
    plt.ylim((0, 24000))
    plt.tight_layout()
    plt.savefig(args.output+'/hom_length_population-wise-temp.png')
    
    # Plotting Variant length population-wise
    fig = plt.figure(figsize =(10, 10))
    ax = fig.add_subplot(111)
    ...
    for flier in bp['fliers']:
        flier.set(marker ='o', color ='black', alpha = 1)    
    ax.set_xticklabels([x for x,_ in color_by_pop.items()])
    ax.set_title('HET SV lengths Population-Wise', fontsize=15)
    plt.yticks(fontsize=15)
    plt.xticks(fontsize=15)
    plt.ylim((0, 24000))
    plt.tight_layout()
    plt.savefig(args.output+'/het_length_population-wise-temp.png')