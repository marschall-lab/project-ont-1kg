...
def get_stats(gaf):
    ...
    logger.info("\tFound %d alignments (where %d are primary alignments) from %d sequences."%(total_alignments, num_prim, len(list(stats.keys()))))
    logger.info("\tAlignment with percentage query length between:")
    logger.info("\t\t0.00 and 0.25: %d"%(num25))
    logger.info("\t\t0.25 and 0.50: %d"%(num50))
    logger.info("\t\t0.50 and 0.75: %d"%(num75))
    logger.info("\t\t0.75 and 0.90: %d"%(num90))
    logger.info("\t\t0.90 and 1.00: %d"%(num100))

    graph(per_read_stat, total_stat, num_align)

def graph(read, total, align):
    ...
    for y_label in read.keys():
        ...
        plt.savefig('/home/samarendra/Pictures/'+y_label+'.png')
        #plt.show()

    fig, axes = plt.subplots(1, 1, figsize=(15, 10))
    axes.hist(align, bins=20, rwidth = 0.8)
    axes.set_title('Number of Alignments per read')
    plt.savefig('/home/samarendra/Pictures/num_align.png')
    #plt.show()
